...
<?php
$url = "http://localhost:8080/portal/activities.php";
$valor=$_GET['n'];

if($valor=='Vin'){
$valor="Vinculacion Comunitaria";
$valor2="2";
$valor3="Vin";}
else
if($valor=='Bienes'){
$valor="Bienestar Estudiantil";
$valor2="3";
$valor3="Bienes";}
else
if($valor=='Actividades'){
$valor="Actividades Institucionales";
$valor2="4";
$valor3="Actividades";}


$rs_noticias=mysql_query("SELECT * FROM noticia where id_tipo_noticia='$valor2'");
...
if ($num_total_registros > 0) {
	...
	$consulta = "SELECT * FROM noticia where id_tipo_noticia=(select id_tipo_noticia from tipo_noticia where nombre_tipo_noticia='".$valor."') ORDER BY id_noticia DESC LIMIT ".$inicio."," . $TAMANO_PAGINA;
	...
	<?php	
		...
		for ($i=1;$i<=$total_paginas;$i++) {
			if ($pagina == $i){
				...
				echo '  <a class="navigation-bar" href="'.$url.'?pagina='.$i.'&n='.$valor3.'">'.$i.'</a>  ';
		}
		if ($pagina != $total_paginas)
			echo '<a class="navigation-arrow-right" href="'.$url.'?pagina='.($pagina+1).'&n='.$valor3.'"></a>';
	}}?>
</div>
</div>
